...
class TensorRTBase():
    def __init__(self, engine_file_path):
        ...
        # Get the name of the first binding (usually input tensor)
        tensor_name = engine.get_tensor_name(0)

        # Get the data type of that tensor
        tensor_dtype = engine.get_tensor_dtype(tensor_name)

        # Convert TRT dtype to NumPy dtype
        self.dtype = trt.nptype(tensor_dtype)
    ...
    def _allocate_buffers(self, engine):
        ...
        for binding in engine:
            ...
            tensor_name = engine.get_tensor_name(binding) if isinstance(binding, int) else binding

            # Get shape (without batch)
            shape = engine.get_tensor_shape(tensor_name)

            # Get dtype
            dtype = trt.nptype(engine.get_tensor_dtype(tensor_name))

            # If using explicit batch, you must provide batch size at runtime
            # So here we assume batch size is known or set to 1 as default
            batch_size = 1  # Change this to your actual batch size at inference
    ...
    def inference(self, input_tensor):
        ...
        input_tensor_name = self.input_names[0]  # assuming 1 input
        output_tensor_names = self.output_names  # list of output tensor names
        ...
        context.set_tensor_address(input_tensor_name, int(cuda_inputs[0]))
        for i, output_name in enumerate(output_tensor_names):
            context.set_tensor_address(output_name, int(cuda_outputs[i]))
