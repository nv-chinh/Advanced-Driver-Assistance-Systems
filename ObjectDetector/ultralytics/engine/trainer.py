...
class BaseTrainer:
    ...
    def save_model(self):
        ...
        if self.best_fitness == self.fitness:
            self.prev_best_epoch = self.current_best_epoch
            self.current_best_epoch = self.epoch
            if self.prev_best_epoch + 1 == self.current_best_epoch:
                try:
                    os.remove(self.current_best_path.as_posix())
                except:
                    pass
            self.current_best_path = self.best.with_name(self.best.stem+'_'+str(self.epoch)+self.best.suffix)
            torch.save(ckpt, self.current_best_path, pickle_module=pickle)
