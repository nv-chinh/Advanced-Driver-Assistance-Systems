...
parser.add_argument('--input_onnx_model', '-i', default="culane_res18.onnx", type=str, help='Onnx model path.')
parser.add_argument('--output_trt_model', '-o', default="culane_res18.trt", type=str, help='Tensorrt model path.')
...
class EngineBuilder:
	...
	def create_engine(self, trt_model_path : str, calib_image_path: Optional[str] = None):
		...
		engine = runtime.deserialize_cuda_engine(serialized_engine)# engine = self.builder.build_serialized_network(network, config) # 已经序列化,类型为:<class 'tensorrt.tensorrt.IHostMemory'
